...
$('#especie.typeahead').typeahead({
    ...
    displayKey: function($keys){
        ...
        if(($keys['varietal'] == null) && ($keys['forma'] == null) && ($keys['subespecie'] == null)){
            return $keys['genero']+' '+$keys['especifico'];
        }else {
            if (($keys['subespecie'] != null)) {
                return $keys['genero'] + ' ' + $keys['especifico'] + ' subsp. ' + $keys['subespecie'];
            } else {
...
$('#especifico.typeahead').typeahead({
    ...
    displayKey: 'nombre',
    templates: {
        header: '<h6 class="type-header">Seleccione un epíteto específico</h6>'
    },
...
var subespecie= new Bloodhound({
    ...
    remote: {
        url: root_url+'buscar/subespecies/%QUERY',
        wildcard: '%QUERY'
    }
});
...
$('#subespecie.typeahead').typeahead({
    hint: false,
    highlight: true,
    minLength: 1
    ...
    displayKey: 'nombre',
    templates: {
        header: '<h6 class="type-header">Seleccione una subespecie</h6>'
    },
    source: subespecie.ttAdapter()

}).bind('typeahead:select', function (ev, suggestion) {
    window.location.replace(root_url+'subespecies/'+suggestion.id);
});
