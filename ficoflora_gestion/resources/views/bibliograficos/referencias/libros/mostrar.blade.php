                ...
                <div class="panel-body">
                    <div class="widget-summary">
                        ...
                        <div class="widget-summary-col">
                            <div class="summary">
                                <div class="info">
                                    <strong class="amount">{{$libro->autores}}, {{$libro->fecha}}{{$libro->letra}}</strong>
                                </div>
                            ...
                            @if($usuario->perfil_id <=3)
                                <div class="summary-footer">
                                    <a class="" href="{{route('usuario.libros')}}"><i class="fa fa-user pr-xs"></i>Mis libros</a>

                                @if($libro->creador_id == $usuario->id || $usuario->perfil_id <=2)

                                |    <a href="{{route('libro.editar', [$libro->id])}}"><i class="fa fa-pencil pr-xs"></i>Editar </a>
                                @endif
                            </div>
                                @endif
                        </div>
            ...
            <section class="panel tabla-mostrar-datos ">
                <div class="panel-body">
                    ...
                    <table class="table table-striped">
                        <tr><td><h5><b>Cita:</b></h5> {{$libro->cita}}, {{$libro->fecha}}{{$libro->letra}}</td></tr>
                        <tr><td><h5><b>Autores:</b></h5> {{$libro->autores}}</td></tr>
                        <tr><td><h5><b>Año:</b></h5> {{$libro->fecha}}</td></tr>
                        <tr><td><h5><b>Título:</b></h5> {{$libro->titulo}}</td></tr>

                        @if($libro->editor != null)
                            <tr><td><h5><b>Editor:</b></h5> {{$libro->editor}}</td></tr>
                            @if($libro->capitulo != null)
                                <tr><td><h5><b>Capítulo:</b></h5> {{$libro->capitulo}}</td></tr>
                                <tr><td><h5><b>Intervalo de páginas:</b></h5> {{$libro->intervalo}}</td></tr>
                            @endif
                        @endif

                        {{--@if($libro->edicion != null)--}}
                            <tr><td><h5><b>Edición:</b></h5> {{$libro->edicion}}</td></tr>
                        {{--@endif--}}

                        {{--@if($libro->editorial != null)--}}
                            <tr><td><h5><b>Editorial:</b></h5> {{$libro->editorial}}</td></tr>
                        {{--@endif--}}

                        <tr><td><h5><b>Lugar:</b></h5> {{$libro->lugar}}</td></tr>
                        <tr><td><h5><b>Páginas:</b></h5> {{$libro->paginas}}</td></tr>
                        <tr><td><h5><b>ISBN:</b></h5> {{$libro->isbn}}</td></tr>
                        <tr><td><h5><b>DOI:</b></h5> {{$libro->enlace}}</td></tr>
                        <tr><td><h5><b>Enlace:</b></h5> {{$libro->archivo}}</td></tr>
                        <tr><td><h5><b>Archivo:</b></h5> {{$libro->archivo}}</td></tr>
                        <tr><td><h5><b>Comentarios:</b></h5> {{$libro->comentarios}}</td></tr>
                            ...
                            <tr>
                                ...
                                <td class="perfil">
                                    <a href="{{route('reporte.mostrar', [$especie['id']])}}">
                                        ({{$libro->cita}}, {{$libro->fecha}}{{$libro->letra}}) | <em>{{$especie['nombre']}}</em> <span class="autores">{{$especie['autor']}}</span>
                                    </a>
                                </td>
...
@section('script_section')
    ...
    <script>
        var referencia = <?php echo $libro->id ?>;
