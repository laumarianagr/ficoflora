<?php 
// este archivo contiene las funciones para abrir la conecion, ejecutar el query y cerrar conexion
...
/*  1. abrir la conexion al smbd y probar que es exitosa la conexion */
/*  2. se selecciona la bdd a utilizar y se verifica el exito en la seleccion */
{
	global $id_c;  // para poder usar una variable global dentro de esta funcion
	
	$id_c = mysql_connect(_servidor, _usuario, _ctr);
	@mysql_query ("SET NAMES 'utf8'");
	//@ para que no retorne un warning
	
	if (!$id_c)  // no se abrio conexion
		return false;
...
{ // indica cantidad de filas obtenidos en la consulta $res, ver pag 67
