...
//            echo "</br> familia -1 </br>";
    ...
    public function establecerEspecie($obj_genero)
    {
        ...
        if(($this->existe_forma == true) && ($this->existe_varietal == true) && ($this->existe_especifico == true) && ($this->existe_genero == true)) {
            $obj_especie = Especie::where('especifico_id', $this->especifico)->conVarietal($this->varietal)->conForma($this->forma)->conGeneroId($obj_genero->id)->first();
        }else{
            $obj_especie = null;
        }
...
//            return new Especie(['nombre' => $this->especie, 'varietal' => $this->varietal, 'forma' => $this->forma, 'autor_id' => $autor_id]);
    ...
    private function guardarNuevoRegistro($obj_phylum, $obj_clase, $obj_subclase, $obj_orden, $obj_familia, $obj_genero, $obj_especie)
    {
        ...
        if($this->existe_especie == false) {
            ...
            $obj_especie = new Especie(['genero_id' => $obj_genero->id, 'especifico_id' => $this->especifico, 'varietal_id' => $this->varietal, 'forma_id' => $this->forma, 'autor_id' => $autor_id]);
